...
export default function Campaigns() {
    ...
    async function handleImageUpload(event) {
        ...
        try {
            ...
            const response = await api.post('/uploads/campaign-banner', formDataUpload, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });

            // Atualizar URL do banner com a URL retornada
            const baseUrl = window.location.origin.replace('5173', '3001'); // Ajustar porta do backend
            setFormData({ ...formData, banner_url: `${baseUrl}${response.data.url}` });
        } catch (error) {
    ...
    return (
        <Box>
                    ...
                    <TableBody>
                        {campaigns.map((campaign) => (
                            <TableRow key={campaign.id}>
                                ...
                                <TableCell>{campaign.discount_percentage ? `${campaign.discount_percentage}%` : '-'}</TableCell>
                                ...
                                <TableCell align="right">
                                    <IconButton onClick={() => handleOpen(campaign)} color="primary">
                                        <Edit />
                                    </IconButton>
            ...
            <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
                <DialogTitle>{currentCampaign ? 'Editar Campanha' : 'Nova Campanha'}</DialogTitle>
                <DialogContent>
